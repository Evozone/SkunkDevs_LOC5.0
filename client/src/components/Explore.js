    ...
    useEffect(() => {
        async function getPosts() {
            try {
                ...
                const postsFromServer = await axios.get(
                    `${process.env.REACT_APP_SERVER_URL}/api/explore/getPostsByFilter/?filter=${filters}`
                );
    ...
    return (
        <>
            ...
            >
                ...
                >
                    <ExploreSearch filters={filters} setFilters={setFilters} mode={mode} setPosts={setPosts} posts={posts} />
                </Box>
